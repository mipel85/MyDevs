...
class LamClubsHomeController extends DefaultModuleController
{
	...
	private function build_table()
	{
		...
		$table_model = new SQLHTMLTableModel(LamClubsSetup::$lamclubs_table, 'table', array(
			...
			$controls ? new HTMLTableColumn($this->lang['common.status'], 'published') : '',
		...
		$table_model->add_filter(new HTMLTableBeginsWithTextSQLFilter('ffam_nb', 'filter1', $this->lang['lamclubs.ffam.number']));
		...
		$table_model->add_filter(new HTMLTableLikeTextSQLFilter('name', 'filter3', $this->lang['common.name']));
		
		$status_list = array(LamClubsItem::PUBLISHED => $this->lang['common.status.approved'], LamClubsItem::NOT_PUBLISHED => $this->lang['common.status.draft']);
		$controls ? $table_model->add_filter(new HTMLTableEqualsFromListSQLFilter('published', 'filter4', $this->lang['common.status'], $status_list)) : '';
		...
		foreach ($result as $row)
		{
			...
			$row = array(
				new HTMLTableRowCell(sprintf("%04d", $item->get_ffam_nb()), $draf_class),
				new HTMLTableRowCell($this->lang['lamclubs.' . $item->get_department()], $draf_class),
				new HTMLTableRowCell($item->get_name(), $draf_class),
				$controls ? new HTMLTableRowCell($item->get_status(), $draf_class) : '',
