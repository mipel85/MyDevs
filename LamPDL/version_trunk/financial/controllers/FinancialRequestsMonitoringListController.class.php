...
class FinancialRequestsMonitoringListController extends DefaultModuleController
{
    ...
    private function build_table($request)
    {
        ...
        while($row = $result_club->fetch()) {
            $clubs[LamclubsService::get_item($row['lamclubs_id'])->get_name()] = LamclubsService::get_item($row['lamclubs_id'])->get_name();
        }
        $table_model->add_filter(new HTMLTableEqualsFromListSQLFilter('name', 'filter3', 'Club', $clubs));        
        ...
        foreach ($result as $row) {
            ...
            $values = array(
                ...
                new HTMLTableRowCell(
                    '<span class="modal-button --target-panel-' . $row['id'] . '"><i class="fa fa-square-plus"></i></i></span>
                    <div id="target-panel-' . $row['id'] . '"class="modal">
                        <div class="close-modal modal-overlay" aria-label="' . $this->lang['common.close'] . '"></div>
                        <div class="modal-content">
                            <div class="align-right"><a href="#" class="error big hide-modal close-modal" aria-label="' . $this->lang['common.close'] . '"><i class="far fa-circle-xmark" aria-hidden="true"></i></a></div>
                            <table>
                                <thead>
                                    <tr>
