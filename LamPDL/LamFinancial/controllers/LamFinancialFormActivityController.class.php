 ...
 */
class LamFinancialFormActivityController extends DefaultModuleController
{
    ...
    {
        LamFinancialService::check_config();
    }
    ...
    private function build_form()
    {
        ...
        //radio buttons
        $nb_requests = LamFinancialService::get_remaining_requests_activity(array('jpo', 'exam'));
        ...
        $fieldset->add_field(new LamClubsFormField('club_infos', $this->lang['lamfinancial.club.infos'], array(), array('description' => LangLoader::get_message('lamclubs.ffam.clue', 'common', 'LamClubs'))));
    ...
    private function get_item()
    {
        if ($this->item === null){
            ...
            if (!empty($id)){
                try {
                    $this->item = LamFinancialService::get_item($id);
                ...
                $this->item = new LamFinancialActivityItem();
            }
        }
        return $this->item;
    }
    ...
    private function save()
    {
        ...
        $id = LamFinancialService::add($activity_item);
    ...
    private function build_response(View $view)
    {
        ...
        $breadcrumb->add($this->lang['lamfinancial.activity.title'], LamFinancialUrlBuilder::activity());
    ...
    private function send_form_email()
    {
        ...
        $item_email->add_recipient(LamFinancialConfig::load()->get_recipient_mail_1());
        $item_email->add_recipient(!empty(LamFinancialConfig::load()->get_recipient_mail_2()) ? LamFinancialConfig::load()->get_recipient_mail_2() : '');
        $item_email->add_recipient(!empty(LamFinancialConfig::load()->get_recipient_mail_3()) ? LamFinancialConfig::load()->get_recipient_mail_3() : '');
    ...
    {
        AppContext::get_response()->redirect(LamFinancialUrlBuilder::home(), $this->lang['lamfinancial.email.sent']);
    }
}
?>
