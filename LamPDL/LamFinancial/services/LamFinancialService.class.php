...
class LamFinancialService
{
    ...
    {
        $result = self::$db_querier->insert(LamFinancialSetup::$lamfinancial, $item->get_properties());
    ...
    public static function delete(int $id)
    {
        ...
        self::$db_querier->delete(LamFinancialSetup::$lamfinancial, 'WHERE id=:id', array('id' => $id));
        self::$db_querier->delete(DB_TABLE_EVENTS, 'WHERE module=:module AND id_in_module=:id', array('module' => 'LamFinancial', 'id' => $id));
    }
    ...
    public static function payment_validation(int $id, int $amount_paid)
    {
        ...
        $now = new Date();
        self::$db_querier->update(LamFinancialSetup::$lamfinancial, array('archived' => '1', 'archived_date' => $now->get_timestamp(), 'amount_paid' => $amount_paid), 'WHERE id=:id', array(
            'id' => $id
        ));
    }
    ...
    public static function get_requests($archived)
    {
		...
		FROM ' . LamFinancialSetup::$lamfinancial . '
