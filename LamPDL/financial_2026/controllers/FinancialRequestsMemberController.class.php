    ...
    private function build_table()
    {
        ...
        foreach ($result as $row) {
            ...
            if ($item->is_authorized_to_delete() || $item->is_authorized_to_edit()) {
                $estimate_file = new LinkHTMLElement(FinancialUrlBuilder::dl_estimate($item->get_id()), '<i class="far fa-lg fa-file-lines"></i>', array('aria-label' => $this->lang['financial.request.estimate.amount']));
                $estimate_file = !empty($item->get_estimate_amount()) ? $estimate_file->display() : '';
