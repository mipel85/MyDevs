...
class FinancialBudgetArchivedController extends DefaultModuleController
{
    ...
    private function build_view(HTTPRequestCustom $request)
    {
        ...
        if (in_array($table, $budget_archive_tables))
        {
            ...
            {
                if (in_array($row['Field'], array('budget_domain', 'budget_type', 'annual_amount', 'real_amount', 'unit_amount', 'quantity', 'real_quantity'))) $this->view->assign_block_vars('thead', array(
        ...
        if (count($budget_archive_tables) > 1 || empty($requested_year)) $this->build_year_form($request);
    }
    ...
    private function build_year_form(HTTPRequestCustom $request)
    {
        ...
        $budget_archive_tables = PersistenceContext::get_dbms_utils()->list_module_tables('financial_budget_');
        if (in_array(PREFIX . 'financial_budget_' . $requested_year, $budget_archive_tables)) $current_year = $requested_year;
        else $current_year = '';
