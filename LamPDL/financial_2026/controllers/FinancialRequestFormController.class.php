...
class FinancialRequestFormController extends DefaultModuleController
{
    ...
    private function build_form($budget_params)
    {
        ...
        if ($budget_params['bill_needed'] != 0)
        ...
        if ($budget_params['use_dl'] && $budget_params['bill_needed'])
            $description .= '<br />' . $this->lang['financial.request.bill'];
        ...
        $fieldset = new FormFieldsetHTML('request','' . $budget_params['budget_domain'] . ' - ' . $budget_params['budget_type']);
        ...
        {
            $fieldset->add_field($estimate_amount = new FormFieldTextEditor('estimate_amount', $this->lang['financial.request.estimate.amount'], $this->get_item()->get_estimate_amount(),
                array(
                    'required'    => true,
                    'description' => $this->lang['financial.request.estimate.amount.clue'], 'class' => 'css-class'
                ),
                // le montant estimé n'accepte que des chiffres entre 2 et 5 
                array(new FormFieldConstraintRegex('`^[0-9]{2,5}$`', '', $this->lang['financial.warning.estimate.amount']))
            ));
            ...
            $fieldset->add_field(new FormFieldUploadFile('invoice_url', $this->lang['financial.request.invoice.url'], $this->get_item()->get_invoice_url()->relative(),
                array(
                    'description' => $this->lang['financial.request.invoice.url.clue'],
                    'class' => 'invoice-wide'
                    )
            ));
        }
    ...
    private function save($budget_params)
    {
        ...
        if ($budget_params['use_dl'])
        {
            ...
            $ffam_nb = $club->get_ffam_nb();
                $item->set_estimate_amount($this->form->get_value('estimate_amount'));
