...
class FinancialSetup extends DefaultModuleSetup
{
    ...
    public static function __static()
    {
        ...
        self::$financial_statement_view = PREFIX . 'statement_view';
    }
    ...
    public static function create_financial_statement_view()
    {
        ...
        $statement_view = 'CREATE VIEW statement_view AS 
                SELECT fb.domain AS Domaine, fr.title as `Type de demande`, lc.ffam_nb as `N° FFAM`, lc.name as Club, mem.display_name as Demandeur,
                FROM_UNIXTIME(fr.creation_date, "%d-%m-%Y") as Date_création,
                FROM_UNIXTIME(fr.event_date, "%d-%m-%Y") as Date_événement,
                fr.amount_paid as `Montant versé`, fb.annual_amount as `Budget prévu`, fb.annual_amount-fb.real_amount as `Budget dépensé`, fb.real_amount as `Budget restant`,
                fr.estimate_url as `lien devis`, fr.invoice_url as `lien facture`
