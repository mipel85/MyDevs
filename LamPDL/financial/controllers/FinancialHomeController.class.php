    ...
    private function build_table()
    {
        ...
        foreach ($budgets as $budget)
        {
            ...
            if ($budget->get_unit_amount() !== '0€' && $budget->get_unit_amount() !== '0%')
            {
                ...
                if ($budget->get_max_amount()) $amount .= '<br />' . StringVars::replace_vars($this->lang['financial.bill.max.amount'], array('max_amount' => $budget->get_max_amount()));
                if ($budget->get_use_dl()) $amount .= $this->lang['financial.bill'];
            }else $amount = '';
            ...
            {
                if ($budget->get_real_quantity() == $budget->get_temp_quantity()) $quantity .= $budget->get_real_quantity() . '/';
                else $quantity .= $budget->get_temp_quantity() . '/';
            ...
            }else $request_button = $this->lang['financial.request.not.available'];
            ...
            if (in_array($budget->get_id(), $this->hide_delete_input)) $table_row->hide_delete_input();
