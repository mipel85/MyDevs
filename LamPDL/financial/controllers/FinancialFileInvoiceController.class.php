...
class FinancialFileInvoiceController extends AbstractController
{
	private $item;
	...
	public function execute(HTTPRequestCustom $request)
	{
		...
		{
			if (Url::check_url_validity($this->item->get_invoice_url()->absolute()) || Url::check_url_validity($this->item->get_invoice_url()->relative()))
			{
				...
				header('Content-Disposition: attachment; filename=' . basename($this->item->get_invoice_url()->absolute()));
                ob_clean();
				set_time_limit(0);
                readfile($this->item->get_invoice_url()->absolute());
                exit;
