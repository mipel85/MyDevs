...
class PlanningItem
{
    ...
    {
        if (!$this->website_url instanceof Url) return new Url('');
    ...
    public function set_properties(array $properties)
    {
        ...
        if ($properties['cancelled']) $this->cancel();
        else $this->uncancel();
        if ($properties['approved']) $this->approve();
        else $this->unapprove();
        ...
        if ($properties['map_displayed']) $this->display_map();
        else $this->hide_map();
    ...
    public function init_default_properties($id_category = Category::ROOT_CATEGORY)
    {
        ...
        if (CategoriesAuthorizationsService::check_authorizations()->write() || CategoriesAuthorizationsService::check_authorizations()->moderation()) $this->approve();
        else $this->unapprove();
    ...
    {
        if (($timestamp % 300) < 150) return $timestamp - ($timestamp % 300);
        else return $timestamp - ($timestamp % 300) + 300;
    }
