...
class PlanningMemberItemsController extends DefaultModuleController
{
    private $member;
    ...
    private function build_table(HTTPRequestCustom $request)
    {
        ...
        $table_model->set_layout_title(
            $this->get_member()->get_id() === AppContext::get_current_user()->get_id() ?
            $this->lang['planning.my.items'] :
            $this->lang['planning.member.items'] . ' ' . $this->get_member()->get_display_name()
        );
        ...
        $table_model->add_permanent_filter('lamclubs_id = ' . LamclubsService::get_user_club($this->get_member()->get_id()));
    ...
    private function generate_response($page = 1)
    {
        ...
        $breadcrumb->add($this->lang['planning.my.items'], PlanningUrlBuilder::display_member_items(AppContext::get_current_user()->get_id()));
    ...
    protected function get_member()
    {
        if ($this->member === null)
        {
            ...
            if (!$this->member) DispatchManager::redirect(PHPBoostErrors::unexisting_element());
        }
        return $this->member;
    }
    ...
    public static function get_view()
    {
        ...
        $object->build_table(AppContext::get_request());
        return $object->view;
    }
}
?>
