...
class PlanningItemFormController extends DefaultModuleController
{
    ...
    private function build_form(HTTPRequestCustom $request)
    {
        ...
        $location = '';
        if (is_array($location_value) && isset($location_value['address'])) $location = $location_value['address'];
        else if (!is_array($location_value)) $location = $location_value;
        ...
        }else $option_fieldset->add_field(new FormFieldShortMultiLineTextEditor('location', $this->lang['planning.location'], $location));
        ...
        if ((!$this->is_new_item && $item->is_authorized_to_add()) || CategoriesAuthorizationsService::check_authorizations($item->get_id_category())->moderation())
        {
            ...
            if (!$this->is_new_item) $publication_fieldset->add_field(new FormFieldCheckbox('cancelled', $this->lang['planning.form.cancel'], $item->is_cancelled()));

            if (CategoriesAuthorizationsService::check_authorizations($item->get_id_category())->moderation()) $publication_fieldset->add_field(new FormFieldCheckbox('approved', $this->lang['form.approve'], $item->is_approved()));
        }
    ...
    private function save()
    {
        ...
        if (CategoriesService::get_categories_manager()->get_categories_cache()->has_categories()) $item->set_id_category($this->form->get_value('id_category')->get_raw_value());
        ...
        if ($this->form->get_value('end_date_enabled')) $item->set_end_date($this->form->get_value('end_date'));
        ...
        if ($this->config->is_googlemaps_available())
        {
            if ($this->form->get_value('map_displayed')) $item->display_map();
            else $item->hide_map();
        }
        ...
        if (CategoriesAuthorizationsService::check_authorizations($item->get_id_category())->moderation())
        {
            ...
            {
                $item->approve();
                if ($item->get_id() !== null) $item->set_update_date(new Date());
            }else $item->unapprove();
        }else if (CategoriesAuthorizationsService::check_authorizations($item->get_id_category())->contribution() && !CategoriesAuthorizationsService::check_authorizations($item->get_id_category())->write()) $item->unapprove();
        ...
        {
            if ($this->form->get_value('cancelled')) $item->cancel();
            else $item->uncancel();
    ...
    private function redirect()
    {
        ...
        {
            if ($this->is_new_item) AppContext::get_response()->redirect(PlanningUrlBuilder::home(), StringVars::replace_vars($this->lang['planning.message.success.add'], array('title' => $title)));
            else AppContext::get_response()->redirect(($this->form->get_value('referrer') ? $this->form->get_value('referrer') : PlanningUrlBuilder::home()), StringVars::replace_vars($this->lang['planning.message.success.edit'], array('title' => $title)));
        }else
        {
            if ($this->is_new_item) AppContext::get_response()->redirect(PlanningUrlBuilder::display_pending_items(), StringVars::replace_vars($this->lang['planning.message.success.add'], array('title' => $title)));
            else AppContext::get_response()->redirect(($this->form->get_value('referrer') ? $this->form->get_value('referrer') : PlanningUrlBuilder::display_pending_items()), StringVars::replace_vars($this->lang['planning.message.success.edit'], array('title' => $title)));
        }
    }
