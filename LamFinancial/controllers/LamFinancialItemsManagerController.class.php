 ...
 */
class LamFinancialItemsManagerController extends DefaultModuleController
{
    private $activity;
    ...
    {
        return new FileTemplate('LamFinancial/LamFinancialItemsManagerController.tpl');
    }
    ...
    {
        $nb_jpo_requests = LamFinancialService::get_requests_number('jpo');
        $nb_exam_requests = LamFinancialService::get_requests_number('exam');
    ...
    private function build_activity_table()
    {
        ...
        $table_model = new SQLHTMLTableModel(LamFinancialSetup::$lam_forms, 'items-manager', $columns, new HTMLTableSortingRule('activity_type', HTMLTableSortingRule::DESC));
    ...
    private function generate_response($page = 1)
    {
        ...
        $graphical_environment->get_seo_meta_data()->set_canonical_url(LamFinancialUrlBuilder::activity_manager());

        $breadcrumb = $graphical_environment->get_breadcrumb();
        $breadcrumb->add($this->lang['lam.jpo'], LamFinancialUrlBuilder::home());
        $breadcrumb->add($this->lang['lam.jpo'], LamFinancialUrlBuilder::activity_manager());
