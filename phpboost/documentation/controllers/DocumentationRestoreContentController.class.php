 ...
 * @version     PHPBoost 6.0 - last update: 2022 12 02
...
class DocumentationRestoreContentController extends DefaultModuleController
{
	public function execute(HTTPRequestCustom $request)
	{
		...
		$this->item_content = $this->get_item_content($request);
		$this->check_authorizations();
		
		foreach($this->item_content as $content)
		{
		...
		AppContext::get_response()->redirect(($request->get_url_referrer() && !TextHelper::strstr($request->get_url_referrer(), DocumentationUrlBuilder::display($this->item->get_category()->get_id(), $this->item->get_category()->get_rewrited_name(), $this->item->get_id(), $this->item->get_rewrited_title())->rel()) ? $request->get_url_referrer() : DocumentationUrlBuilder::home()), StringVars::replace_vars($this->lang['documentation.message.success.restore'], array('content' => $request->get_int('content_id'),'title' => $this->item->get_title())));
	}
