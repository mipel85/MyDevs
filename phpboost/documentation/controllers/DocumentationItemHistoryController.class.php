...
class DocumentationItemHistoryController extends DefaultModuleController
{
	...
	public function build_view(HTTPRequestCustom $request)
	{
		...
		$this->view->put_all(array(
			...
			'C_CONTROLS'                 => DocumentationAuthorizationsService::check_authorizations($this->get_item()->get_category()->get_id())->moderation(),

			'ITEM_TITLE' => $this->get_item()->get_title(),
		...
		while ($row = $result->fetch())
		{
			...
			$this->view->assign_block_vars('items', array_merge($item->get_template_vars(), array(
				'C_ACTIVE_CONTENT' => $item->get_item_content()->get_active_content() == 1,
				'U_ARCHIVE' 	   => DocumentationUrlBuilder::archive($item->get_id(), $item->get_item_content()->get_content_id())->rel(),
	...
	private function generate_response(HTTPRequestCustom $request)
	{
		...
		$graphical_environment->get_seo_meta_data()->set_description(StringVars::replace_vars($this->lang['documentation.seo.description.history'], array('item' => $item->get_title())));
		$graphical_environment->get_seo_meta_data()->set_canonical_url(DocumentationUrlBuilder::history($item->get_id()));

		$breadcrumb = $graphical_environment->get_breadcrumb();
		$breadcrumb->add($this->lang['documentation.module.title'], DocumentationUrlBuilder::home());
		$breadcrumb->add($item->get_title(), DocumentationUrlBuilder::display($item->get_category()->get_id(), $item->get_category()->get_rewrited_name(), $item->get_id(), $item->get_rewrited_title()));
		$breadcrumb->add($page_title, DocumentationUrlBuilder::history($item->get_id()));
