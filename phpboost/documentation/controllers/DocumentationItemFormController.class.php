...
class DocumentationItemFormController extends DefaultModuleController
{
	...
	private function build_form(HTTPRequestCustom $request)
	{
		...
		$options_fieldset->add_field(new FormFieldThumbnail('thumbnail', $this->lang['form.thumbnail'], $this->get_item()->get_thumbnail()->relative(), DocumentationItem::THUMBNAIL_URL));
	...
	private function save()
	{
		...
		if ($this->config->is_author_displayed())
			$item->set_author_custom_name(($this->form->get_value('author_custom_name') && $this->form->get_value('author_custom_name') !== $item_content->get_author_user()->get_display_name() ? $this->form->get_value('author_custom_name') : ''));
	...
	private function redirect()
	{
		...
		{
			if ($this->is_new_item)
				AppContext::get_response()->redirect(DocumentationUrlBuilder::display($category->get_id(), $category->get_rewrited_name(), $item->get_id(), $item->get_rewrited_title()), StringVars::replace_vars($this->lang['documentation.message.success.add'], array('title' => $item->get_title())));
			else
				AppContext::get_response()->redirect(($this->form->get_value('referrer') ? $this->form->get_value('referrer') : DocumentationUrlBuilder::display($category->get_id(), $category->get_rewrited_name(), $item->get_id(), $item->get_rewrited_title())), StringVars::replace_vars($this->lang['documentation.message.success.edit'], array('title' => $item->get_title())));
		}
		else
		{
			if ($this->is_new_item)
				AppContext::get_response()->redirect(DocumentationUrlBuilder::display_pending(), StringVars::replace_vars($this->lang['documentation.message.success.add'], array('title' => $item->get_title())));
			else
				AppContext::get_response()->redirect(($this->form->get_value('referrer') ? $this->form->get_value('referrer') : DocumentationUrlBuilder::display_pending()), StringVars::replace_vars($this->lang['documentation.message.success.edit'], array('title' => $item->get_title())));
		}
	}
	...
	private function generate_response(View $view)
	{
		...
		else
		{
			...
			$category = $item->get_category();
			$breadcrumb->add($item->get_title(), DocumentationUrlBuilder::display($category->get_id(), $category->get_rewrited_name(), $item->get_id(), $item->get_rewrited_title()));
