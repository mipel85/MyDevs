...
switch($actions)
{
    case 'insert_teams':
        ...
        foreach (build_teams($players) as $team) {
            echo '<pre>';
            var_dump($party_id, $round_id, $team);
            echo '</pre>';
            $insert = new Teams();
            ...
            if (array_key_exists(1, $team)) {
                $player_2 = explode(':', $team[1]);
                $insert->set_player_2_id(array_shift($player_2));
                $insert->set_player_2_name(array_pop($player_2));
            }
            if (array_key_exists(2, $team)) {
                $player_3 = explode(':', $team[2]);
                $insert->set_player_3_id(array_shift($player_3));
                $insert->set_player_3_name(array_pop($player_3));
            }
            $insert->add_team();
        }
        break;
